 ...
 * for a the camus.message.timestamp.field, convert that timestamp to a unix
 * epoch long using camus.message.timestamp.format, and then set the
 * CamusWrapper's timestamp property to the record's timestamp. If the JSON does
 * not have a timestamp or if the timestamp could not be parsed properly, then
